...
class ScrolledPanel(Window):
    def __init__(self, parent, id=wx.ID_ANY, pos=wx.DefaultPosition,
        ...
        self.Unbind(wx.EVT_PAINT, handler=self._on_paint)
        
        self.Unbind(wx.EVT_ENTER_WINDOW, handler=self._on_enter_window)
        self.Unbind(wx.EVT_LEAVE_WINDOW, handler=self._on_leave_window)

        self.Unbind(wx.EVT_LEFT_DCLICK, handler=self._on_left_down)
        self.Unbind(wx.EVT_LEFT_DOWN, handler=self._on_left_down)
        self.Unbind(wx.EVT_LEFT_UP, handler=self._on_left_up)

        self.__scrolled_panel.Bind(wx.EVT_MOUSEWHEEL, self._on_mousewheel)

        self.__scrollbar_window_vertical.Bind(wx.EVT_PAINT, self._on_paint)
        self.__scrollbar_window_vertical.Bind(wx.EVT_LEFT_DOWN, self._on_left_down)
        self.__scrollbar_window_vertical.Bind(wx.EVT_LEFT_UP, self._on_left_up)
        self.__scrollbar_window_vertical.Bind(wx.EVT_LEAVE_WINDOW, self._on_leave_window)
        self.__scrollbar_window_vertical.Bind(wx.EVT_MOTION, self._on_motion)

        self.__scrollbar_window_horizontal.Bind(wx.EVT_PAINT, self._on_paint)
        self.__scrollbar_window_horizontal.Bind(wx.EVT_LEFT_DOWN, self._on_left_down)
        self.__scrollbar_window_horizontal.Bind(wx.EVT_LEFT_UP, self._on_left_up)
        self.__scrollbar_window_horizontal.Bind(wx.EVT_LEAVE_WINDOW, self._on_leave_window)
        self.__scrollbar_window_horizontal.Bind(wx.EVT_MOTION, self._on_motion)
    ...
    def _on_paint(self, event: wx.Event) -> None:
        ...
        client_size = self.__scrolled_panel.GetClientSize()
        virtual_size = self.__scrolled_panel.GetVirtualSize()
        ...
        # width)
        scrollbar_window_client_size = scrollbar_window.GetClientSize()
        ...
        view_from_start = self.__scrolled_panel.CalcScrolledPosition(0, 0)
        ...
        gcdc, gc = self._get_drawing_contexts(scrollbar_window)
        gcdc.Clear()

        drawing_rect: wx.Rect = scrollbar_window.GetClientRect()
        ...
        if scrollbar_window_is_vertical:
            ...
            scrollbar_thumb_width = self._scrollbar_width
            ...
            scrollbar_thumb_height = self._scrollbar_width
            # drawn rectangle
            scrollbar_thumb_drawn_x = scrollbar_thumb_x + padding
            scrollbar_thumb_drawn_y = scrollbar_thumb_y
            scrollbar_thumb_drawn_width = scrollbar_thumb_width - 2 * padding
